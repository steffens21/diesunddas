...
def main():
    ...
    dictGames = defaultdict(int)
    dictCorrect = defaultdict(int)
    ...
    for data in listRawData:
        for sBettingAgent in dictOddCol.keys():
            ...
            if sResult == min(tupOdds, key=lambda x: x[1])[0]:
                dictCorrect[sBettingAgent] += 1
        
            dictGames[sBettingAgent] += 1

    print '\t'.join(sorted(dictOddCol.keys()))
    print '\t'.join([ "{0:.2f}".format(dictCorrect[sBetAgent] / float(dictGames[sBetAgent]) * 100) for sBetAgent in sorted(dictOddCol.keys())])
